...
class InMemoryIdentityServiceTests {
    ...
    fun `assert ownership`() {
        ...
        val certStore = CertStore.getInstance("Collection", CollectionCertStoreParameters(setOf(bobRootCert)))
        val bobCertPath = X509Utilities.createCertificatePath(bobRootCert.cert, certStore, bobTxCert.cert, revocationEnabled = false)
        val bob = PartyAndCertificate(BOB.name, bobRootKey.public, bobRootCert, bobCertPath)

        // Now we have identities, construct the service and let it know about both
        val service = InMemoryIdentityService(setOf(alice, bob), emptyMap(), DUMMY_CA.certificate.cert)
    ...
    private fun createParty(x500Name: X500Name): Triple<KeyPair, CertPath, PartyAndCertificate> {
        ...
        val certStore = CertStore.getInstance("Collection", CollectionCertStoreParameters(setOf(rootCert.cert)))
        val certPath = X509Utilities.createCertificatePath(rootCert.cert, certStore, txCert.cert, revocationEnabled = false)
